...
游戏中的各种交互式设施（Actor）是角色工作和生活的核心场所，每个设施都有特定的功能和用途。

#### 生产设施

| 设施名称 | 主要功能 | 使用角色 | 相关操作 |
|---------|---------|---------|---------|
| **CultivateChamber**<br>（培养舱） | 种植和培育作物 | Farmer | - 种植Cotton（棉花）<br>- 种植Corn（玉米）<br>- 等待作物成熟<br>- 收获成熟作物 |
| **WorkStation**<br>（工作台） | 加工原材料和制作商品 | Crafter | - 制作Thread（棉线）<br>- 制作Cloth（布料）<br>- 制作Coat（衣服） |
| **Stove**<br>（炉灶） | 烹饪食物 | Chef | - 烹饪Meal（套餐） |

#### 生活设施

| 设施名称 | 主要功能 | 使用角色 | 相关操作 |
|---------|---------|---------|---------|
| **Bed**<br>（床铺） | 恢复精力值（Energy） | 所有角色 | - Sleep（睡觉）<br>- 每个角色通常有专属的床位 |
| **Table**<br>（餐桌） | 进食场所 | 所有角色 | - Eat（进食）<br>- 消耗Meal恢复Hunger值 |

#### 存储设施

| 设施名称 | 主要功能 | 使用角色 | 相关操作 |
|---------|---------|---------|---------|
| **Storage**<br>（仓库） | 集中存储所有物资 | 所有角色 | - Take（拿取物品）<br>- Put（存放物品）<br>- 查看库存状态 |

#### 设施详细说明

**🌱 CultivateChamber（培养舱）**
- **核心属性**：
  - `CultivatePhase`：当前阶段（等待种植/生长中/等待收获）
  - `TargetCultivateType`：目标作物类型（Cotton/Corn）
  - `GrowthProgress`：作物生长进度
  - `WorkProgress`：当前工作进度
  - `HasWorker`：是否有角色正在工作
- **工作流程**：
  1. Farmer选择作物类型（Cotton或Corn）
  2. 执行种植操作（Use指令，ParamID指定作物类型）
  3. 作物自动生长（GrowthProgress增长）
  4. 成熟后Farmer收获，产出对应物品
- **特点**：支持自动化生长，种植后无需持续操作

**🔧 WorkStation（工作台）**
- **核心功能**：Crafter的核心工作场所，所有材料加工和商品制作都在此完成
- **支持的配方**：
  - Thread（棉线）：5个Cotton → 1个Thread
  - Cloth（布料）：5个Cotton → 1个Cloth
  - Coat（衣服）：2个Cloth + 3个Thread → 1个Coat
- **工作流程**：
  1. Crafter将原料放入WorkStation
  2. 执行Use指令（ParamID指定配方ID）
  3. 完成工作量后产出成品
- **特点**：可存放中间产物，支持复杂的多步骤制作

**🍳 Stove（炉灶）**
- **核心功能**：Chef的专属设施，用于烹饪食物
- **支持的配方**：
  - Meal（套餐）：5个Corn → 1个Meal
- **工作流程**：
  1. Chef将Corn放入Stove
  2. 执行烹饪操作
  3. 完成后产出Meal
- **特点**：Meal是游戏中唯一的食物来源，Stove的效率直接影响殖民地存活

**🛏️ Bed（床铺）**
- **核心功能**：角色休息恢复精力的场所
- **使用机制**：
  - 每个角色通常有专属的床位（如Bed_1分配给Farmer）
  - 当Energy值过低时，角色应该Sleep
  - 睡眠会持续恢复Energy直到满值
- **特点**：睡眠是被动恢复过程，角色在床上时会自动恢复精力
...
- **使用机制**：
  - 角色携带或从Storage获取Meal
  - 在Table处执行Eat操作
  - 消耗1个Meal，恢复80点Hunger值
  - 进食需要约20个游戏时间单位
- **特点**：集中用餐地点，未来可扩展为社交场所
...
- **存储内容**：
  ...
  - 成品：Meal、Coat
- **操作接口**：
  - `Take(Storage, ItemID, Count)`：拿取指定物品
  - `Put(Storage, ItemID, Count)`：存放指定物品
- **重要性**：
  - 所有生产活动的起点和终点
  - 角色间共享资源的枢纽
  - LLM决策时需要检查Storage库存来规划行动
- **特点**：无容量限制（当前版本），所有角色都可访问
...
Storage（中央仓库）
    ...
    ↓
┌──────────────┬──────────────┬──────────────┐
│ CultivateChamber │  WorkStation  │    Stove    │
│  种植作物    │  加工材料    │   烹饪食物   │
└──────┬───────┴──────┬───────┴──────┬───────┘
       │              │              │
       ↓ 收获        ↓ 制作成品     ↓ 烹饪完成
